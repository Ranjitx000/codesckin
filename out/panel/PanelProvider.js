...
const ColorExtractor_1 = require("../color/ColorExtractor");
// ─── JS runtime version ──────────────────────────────────────────────────────
//
...
const REGION_CONFIG_MAP = new Map(REGION_CONFIGS.map(c => [c.id, c]));
...
const fsPathToVscodeUri = (fsPath) => {
...
class PanelProvider {
    ...
    constructor(panel, context) {
        ...
        this._cssManager = new CssManager_1.CssManager(context);
    ...
    static createOrShow(context) {
        ...
        const panel = vscode.window.createWebviewPanel('codeskinPanel', 'CodeSkin', { viewColumn: column, preserveFocus: true }, {
            enableScripts: true,
     ...
     */
    static get jsVersion() { return CODESKIN_JS_VERSION; }
    // ── State persistence ─────────────────────────────────────────────────────
    _loadPersistedState() {
        const saved = this._context.globalState.get('codeskin.state');
     ...
     */
    async _persistState() {
        await this._context.globalState.update('codeskin.state', structuredClone(this._state));
    }
    // ── Message handler ───────────────────────────────────────────────────────
    async _handleMessage(message) {
        ...
        switch (message.command) {
            ...
            case 'EXTRACT_COLORS':
                ...
                break;
            default:
                console.warn('[CodeSkin] Unhandled command:', message.command);
        }
    }
    // ── Cache invalidation ────────────────────────────────────────────────────
    /**
     ...
     */
    _invalidateCache(region) {
        this._dirtyRegions.add(region);
    }
    // ── Message sub-handlers ──────────────────────────────────────────────────
    async _handlePickFile(region, autoSync) {
        ...
        if (autoSync) {
            const [, colorOk] = await Promise.all([
                applyPromise,
                ColorExtractor_1.ColorExtractor.extractAndApply(savedPath),
            ]);
            if (!colorOk) {
                vscode.window.showWarningMessage('CodeSkin: Color extraction failed — see Output panel.');
            }
        }
        else {
            await applyPromise;
    ...
    async _handleCopyImage(fromRegion, toRegion) {
    ...
    async _handleExtractColors(region) {
        const imgPath = this._state[region].imagePath;
        if (!imgPath) {
            vscode.window.showWarningMessage(`CodeSkin: No wallpaper for ${region}. Upload an image first.`);
            return;
        }
        const ok = await ColorExtractor_1.ColorExtractor.extractAndApply(imgPath);
        if (ok) {
            vscode.window.showInformationMessage('CodeSkin: Colors extracted ✓  Reload to see theme change.');
        }
        else {
            vscode.window.showErrorMessage('CodeSkin: Color extraction failed. Check the Output panel.');
        }
    }
    // ── Apply pipeline ────────────────────────────────────────────────────────
    /**
    ...
    async _applyNow(region) {
        ...
        if (!this._installed) {
            ...
            const action = await vscode.window.showInformationMessage(`CodeSkin: ${label} background applied. Reload window to activate.`, 'Reload Window', 'Later');
    ...
    _writeTick() {
        ...
        fs.promises.writeFile(tickPath, String(this._tick)).catch(() => { });
    }
    ...
    async repairAfterVscodeUpdate() {
        ...
        this._installed = true;
        const action = await vscode.window.showInformationMessage('CodeSkin: VS Code updated — backgrounds re-applied. Reload to activate.', 'Reload Window', 'Later');
        if (action === 'Reload Window') {
            await vscode.commands.executeCommand('workbench.action.reloadWindow');
        }
    }
    /** Show a user-friendly error based on the typed InstallResult. */
    _showInstallError(result) {
        if (result.reason === 'permission') {
            vscode.window.showErrorMessage(`CodeSkin: ${result.message}`, 'OK');
        }
        else {
     ...
     */
    async _writeIfChanged(filePath, content) {
    ...
    _buildVideoConfigs() {
        ...
        for (const cfg of REGION_CONFIGS) {
            ...
            if (!s.enabled || !s.imagePath || !s.isVideo) {
                continue;
            }
            result[cfg.id] = {
    ...
    _generateJs(videoConfigs) {
        ...
        if (!rawTickPath.startsWith('/')) {
            rawTickPath = '/' + rawTickPath;
        }
    ...
    _pushStateToWebview() {
        ...
        for (const r of ALL_REGIONS) {
            ...
            if (s.imagePath === null) {
                webviewUri = null;
                if (cached !== null) {
                    this._lastWebviewUri.set(r, null);
                }
            }
            else {
    ...
    _buildHtml(webview) {
        ...
        try {
            ...
            html = html.replace(/(href|src)="(\/[^"]+)"/g, (_m, attr, assetPath) => {
                const abs = path.join(distDir, assetPath);
                return fs.existsSync(abs)
                    ? `${attr}="${webview.asWebviewUri(vscode.Uri.file(abs))}"`
                    : _m;
            });
            ...
            return (`<!DOCTYPE html><html><body style="color:#ccc;padding:2rem">` +
